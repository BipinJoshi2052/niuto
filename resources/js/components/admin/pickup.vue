...
<div>
                            ...
                            <div class="card card-custom gutter-b bg-transparent shadow-none border-0">
                                <div class="card-header align-items-center  border-bottom-dark px-0">
                                    ...
                                    <div class="icons d-flex">
                                        <button class="btn ml-2 p-0 kt_notes_panel_toggle" data-toggle="tooltip" title="" data-placement="right" data-original-title="Check out more demos" v-if="$parent.permissions.includes('product-pickup-list')">
                                ...
                                <div class="card-body">
                                    <div>
                                        <div class=" table-responsive" id="printableTable">
                                            ...
                                            <div id="productpickupTable_wrapper" class="dataTables_wrapper no-footer">

                                            <div class="dataTables_length" id="productpickupTable_length"><label>Show 
                                            <select name="productpickupTable_length" aria-controls="productpickupTable" class="" v-model="limit" v-on:change="fetchpickups()">
                                            ...
                                            <div id="productpickupTable_filter" class="dataTables_filter"><label>Search:<input type="search" class="" placeholder="" aria-controls="productpickupTable" v-model="searchParameter" @keyup="fetchpickups()"></label></div>
                                                <table id="productpickupTable" class="display dataTable no-footer" role="grid">
                                                    ...
                                                    <tbody class="kt-table-tbody text-dark">
                                                        <tr class="kt-table-row kt-table-row-level-0 odd" role="row" v-for="pickup in pickups" v-bind:key="pickup.pickup_id">
                                                            <td class="sorting_1">
                                                                {{pickup.id}}
                                                            </td>
                                                            <td>
                                                ...
                                                <ul class="pagination pagination-sm m-0 float-right">
                                                    <li v-bind:class="[{disabled: !pagination.prev_page_url}]"><button class="page-link" href="#" @click="fetchpickups(pagination.prev_page_url)">Previous</button></li>

                                                    <li class="disabled"><button class="page-link text-dark" href="#">Page {{ pagination.current_page }} of {{ pagination.last_page }}</button></li>

                                                    <li v-bind:class="[{disabled: !pagination.next_page_url}]" class="page-item"><button class="page-link" href="#" @click="fetchpickups(pagination.next_page_url)">Next</button></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
    ...
    <div class="offcanvas offcanvas-right kt-color-panel p-5 kt_notes_panel" v-if="display_form" :class="display_form ? 'offcanvas-on' : ''">
        <div class="offcanvas-header d-flex align-items-center justify-content-between pb-3">
            <h4 class="font-size-h4 font-weight-bold m-0">Add Pickup</h4>
        ...
        <form id="myform">
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="form-group " v-for="(language,index) in languages" v-if="language.id == selectedLanguage">
                        ...
                        <input type="text" :name="'name'+index" v-model="pickup.name[index]" class="form-control" />
                        <small class="form-text text-danger" v-if="errors.has('name')" v-text="errors.get('name')"></small>
                    </div>
                    <div class="form-group " v-for="(language,index) in languages" v-if="language.id == selectedLanguage">
                        <label class="text-dark">Country ( {{ language.language_name }} ) </label>
                        <input type="text" :name="'country'+index" v-model="pickup.country[index]" class="form-control" />
                        <small class="form-text text-danger" v-if="errors.has('country')" v-text="errors.get('country')"></small>
                    </div>
                    <div class="form-group " v-for="(language,index) in languages" v-if="language.id == selectedLanguage">
                        <label class="text-dark">State ( {{ language.language_name }} ) </label>
                        <input type="text" :name="'state'+index" v-model="pickup.state[index]" class="form-control" />
                    ...
                    <div class="form-group " v-for="(language,index) in languages" v-if="language.id == selectedLanguage">
                        <label class="text-dark">Postal Code ( {{ language.language_name }} ) </label>
                        <input type="text" :name="'postalcode'+index" v-model="pickup.postalcode[index]" class="form-control" />
                        <small class="form-text text-danger" v-if="errors.has('postalcode')" v-text="errors.get('postalcode')"></small>
                    </div>
                    <div class="form-group ">
                        <label>Status</label>
                        <fieldset class="form-group mb-3">
                            <select class="js-example-basic-single js-states form-control bg-transparent" v-model="pickup.is_active">
            ...
            <button type="button" @click="addUpdatepickup()" class="btn btn-primary">Submit</button>
        </form>
    </div>
</div>
</template>
...
export default {
    ...
    methods: {
                ...
                .then(res => {
                    if (res.data.status == "Success") {
                        ...
                        for(var i = 0 ; i < res.data.data.length; i++){
                            this.pickup.language_id.push(res.data.data[i].id);
        ...
        fetchpickups(page_url) {
            this.$parent.loading = true;
            let vm = this;
            page_url = page_url || "/api/admin/pickup";
        ...
        deletepickup(id) {
            if (confirm('Are You Sure?')) {
                this.$parent.loading = true;
                axios.delete(`/api/admin/pickup/${id}`,this.token)
                    .then(res => {
                        if (res.data.status == "Success") {
        ...
        addUpdatepickup() {
            this.$parent.loading = true;
            var url = '/api/admin/pickup';
            if (this.edit === false) {
                // Add
                this.request_method = 'post'
            } else {
                // Update
                var url = '/api/admin/pickup/' + this.pickup.id;
                this.request_method = 'put'
            }
            axios[this.request_method](url, this.pickup, this.token)
                .then(res => {
                    if (res.data.status == "Success") {
                        this.display_form = 0;
                        this.$toaster.success('Settings has been updated successfully')
